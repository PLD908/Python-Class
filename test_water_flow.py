#This file contains test functions that verify the correctness of the water pressure calculation functions in the water_flow.py program.

#Assignment Grading Category: 5 (Exceeds requirements)
#Justification: The test suite not only includes all required test functions with the specified test cases, but also contains additional test functions for the kPa to PSI conversion feature, ensuring comprehensive verification of both required and extra functionality.
...
def test_kpa_to_psi():
    ...
    assert kpa_to_psi(200.0) == approx(29.0076, abs=0.001)
    assert kpa_to_psi(300.0) == approx(43.5114, abs=0.001)
    assert kpa_to_psi(400.0) == approx(58.0152, abs=0.001)
